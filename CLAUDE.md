...
- Go 1.25 (with `iter.Seq`, `iter.Seq2`, range-over-func) + `gqlparser/v2` (GraphQL AST), `duckdb-go/v2` (build tag: `duckdb_arrow`) + `hugr-lab/query-engine` (engine, client, db provider, catalog), `mark3labs/mcp-go` (MCP server) (002-cluster-mode)
- PostgreSQL (shared CoreDB in cluster mode), DuckDB (local per-node) (002-cluster-mode)
...
## Recent Changes
- 002-cluster-mode: Added Go 1.25 (with `iter.Seq`, `iter.Seq2`, range-over-func) + `gqlparser/v2` (GraphQL AST), `duckdb-go/v2` (build tag: `duckdb_arrow`) + `hugr-lab/query-engine` (engine, client, db provider, catalog), `mark3labs/mcp-go` (MCP server)
