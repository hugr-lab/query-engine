#!/bin/bash
# CoreDB integration test runner.
# Runs schema init and migration tests against DuckDB (in-memory) and PostgreSQL (Docker).
#
# Usage:
...
PG_DSN="postgres://test:test@localhost:5434/coredb_test?sslmode=disable"
...
cd "$PROJECT_ROOT"
CGO_CFLAGS="-O1 -g" go test -tags=duckdb_arrow -v -count=1 -run "TestDuckDB" ./integration-test/coredb/
echo ""
echo "DuckDB tests passed."
...
COREDB_TEST_PG_DSN="$PG_DSN" CGO_CFLAGS="-O1 -g" go test -tags=duckdb_arrow -v -count=1 -run "TestPostgres" ./integration-test/coredb/
