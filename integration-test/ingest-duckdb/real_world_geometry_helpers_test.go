...
type realWorldFeature = ingesttest.RealWorldFeature
type realWorldGeometry = ingesttest.RealWorldGeometry
type realWorldExpectedCounts = ingesttest.RealWorldExpectedCounts
...
func ingestNaturalEarthGeometrySingleStream(t *testing.T, env *ingestEnv, features []realWorldFeature) int {
	t.Helper()

	return ingestRealWorldGeometrySingleStream(t, env, "natural-earth", features, geometryArrowFields())
}

func ingestNaturalEarthGeometryFixedSizeListSingleStream(t *testing.T, env *ingestEnv, features []realWorldFeature) int {
	t.Helper()
...
func ingestRealWorldGeometrySingleStream(t *testing.T, env *ingestEnv, rowPrefix string, features []realWorldFeature, geometryFields []arrow.Field) int {
	t.Helper()
...
func appendNaturalEarthGeometryTypesRow(t *testing.T, b *array.RecordBuilder, rowPrefix string, row int, feature realWorldFeature) {
	t.Helper()
...
func assertNaturalEarthGeometrySamples(t *testing.T, db *sql.DB, features []realWorldFeature) {
	t.Helper()
...
func assertNaturalEarthGeometryValues(t *testing.T, db *sql.DB, features []realWorldFeature) {
	t.Helper()
...
func assertNaturalEarthColumnValuesEqual(t *testing.T, db *sql.DB, rowPrefix string, sample naturalEarthSample, expectedByColumn map[string]string) {
	...
	for _, col := range columns {
		selects = append(selects, "ST_Equals("+col+", ST_GeomFromText(CAST(? AS VARCHAR)))")
