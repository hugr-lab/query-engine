...
func newStaticProvider(t *testing.T, sdl string) catalog.Provider {
	t.Helper()
...
func TestParseQuery_SimpleQuery(t *testing.T) {
	...
	op, err := catalog.ParseQuery(context.Background(), p, `query { user(id: "1") { id name } }`, nil)
	require.NoError(t, err)
	require.NotNil(t, op)
...
func TestParseQuery_WithVariables(t *testing.T) {
	...
	op, err := catalog.ParseQuery(context.Background(), p,
...
func TestParseQuery_InvalidQuery(t *testing.T) {
	...
	_, err := catalog.ParseQuery(context.Background(), p, `query {`, nil)
	require.Error(t, err)
}
...
func TestParseQuery_ValidationError(t *testing.T) {
	...
	_, err := catalog.ParseQuery(context.Background(), p,
...
func TestParseQuery_OperationNotFound(t *testing.T) {
	...
	_, err := catalog.ParseQuery(context.Background(), p,
		`query A { user { id } }`, nil,
		catalog.WithOperationName("NonExistent"))
	require.Error(t, err)
