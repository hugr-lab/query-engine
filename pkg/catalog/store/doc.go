...
//
// It is the engine's catalog storage. The storage it replaced shredded a FULLY
// compiled GraphQL schema into the _schema_* tables; this one stores only the
// BASE entities that arrive from sources (modules, data objects, their fields,
...
// cached with per-data-source invalidation (schemaCache): a reconcile evicts
// only the changed/removed sources.
//
