...
// Source is a runtime source that registers CoreDB management functions.
// These are mutation functions for export, import, description sync,
// index recreation, and schema reset operations.
type Source struct {
	db *db.Pool
}
...
func (s *Source) Name() string          { return "core.db" }
...
func (s *Source) Attach(ctx context.Context, pool *db.Pool) error {
	s.db = pool
	return s.registerFunctions(ctx)
}
