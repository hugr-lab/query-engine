...
func h3RootNode(ctx context.Context, provider catalog.Provider, planner Catalog, query *ast.Field, vars map[string]any) (*QueryPlanNode, error) {
...
func h3DataNode(ctx context.Context, defs base.DefinitionsSource, planner Catalog, query *ast.Field, vars map[string]any) (*QueryPlanNode, error) {
	...
	if !ok || resolution < 1 || resolution > 15 {
		return nil, sdl.ErrorPosf(query.Position, "H3 query 'resolution' argument must be an integer between 1 and 15")
	}
	...
	for _, f := range engines.SelectedFields(query.SelectionSet) {
		...
		case base.H3DataFieldName:
			...
			if err != nil {
				return nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' sub query: %v", query.Alias, err)
			}
		...
		case base.DistributionFieldName, base.BucketDistributionFieldName:
			...
			if err != nil {
				return nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field '%s' distribution by: %v", query.Alias, f.Field.Alias, err)
			}
...
func h3DataSubQueryNodes(ctx context.Context, defs base.DefinitionsSource, planner Catalog, query *ast.Field, res int, vars map[string]any) (out QueryPlanNodes, inner []string, err error) {
	...
	for _, f := range ss {
		...
		if len(am) == 0 {
			return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' must contain field arguments", query.Alias)
		}
		v, ok := am["field"]
		if !ok {
			return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' must contain 'field' argument", query.Alias)
		}
		baseFieldName, ok := v.(string)
		if !ok || baseFieldName == "" {
			return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' argument 'field' must be a string", query.Alias)
		}
		...
		switch {
		case sdl.IsAggregateQuery(f.Field) || sdl.IsBucketAggregateQuery(f.Field):
			baseNode, _, err = aggregateDataNode(ctx, defs, planner, true, f.Field, vars)
			if err != nil {
				return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' aggregate sub query: %v", query.Alias, err)
			}
		case sdl.IsSelectQuery(f.Field) || sdl.IsSelectOneQuery(f.Field):
			baseNode, _, err = selectDataObjectNode(ctx, defs, planner, f.Field, vars)
			if err != nil {
				return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' field 'data' select sub query: %v", query.Alias, err)
			}
		default:
			return nil, nil, sdl.ErrorPosf(f.Field.Position, "H3 query '%s' unsupported query type for 'data' field: %s", query.Alias, f.Field.Name)
		}
...
func h3distributionByNode(ctx context.Context, defs base.DefinitionsSource, planner Catalog, h3Query, query *ast.Field, vars map[string]any) (*QueryPlanNode, error) {
	...
	if err != nil {
		return nil, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' distribution by parameters: %v", h3Query.Alias, query.Alias, err)
	}
	if dp.numeratorQuery == nil || dp.denominatorQuery == nil {
		return nil, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' distribution by numerator and denominator queries must be specified", h3Query.Alias, query.Alias)
	}
...
func h3distributionByParams(h3Query, query *ast.Field, vars map[string]any) (params h3DistributionByParams, err error) {
	...
	if len(am) == 0 {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' must contain arguments", h3Query.Alias, query.Alias)
	}
	v, ok := am["numerator"]
	if !ok {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' must contain 'numerator' argument", h3Query.Alias, query.Alias)
	}
	numerator, ok := v.(string)
	if !ok || numerator == "" {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'numerator' must be a string", h3Query.Alias, query.Alias)
	}
	v, ok = am["denominator"]
	if !ok {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' must contain 'denominator' argument", h3Query.Alias, query.Alias)
	}
	denominator, ok := v.(string)
	if !ok || denominator == "" {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'denominator' must be a string", h3Query.Alias, query.Alias)
	}
	...
	if numeratorQuery == nil ||
		!sdl.IsAggregateQuery(numeratorQuery.Field) &&
			!sdl.IsBucketAggregateQuery(numeratorQuery.Field) {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'numerator' must point to the aggregation value in the 'data' field", h3Query.Alias, query.Alias)
	}
	...
	if denominatorQuery == nil ||
		!sdl.IsAggregateQuery(denominatorQuery.Field) &&
			!sdl.IsBucketAggregateQuery(denominatorQuery.Field) {
		return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'denominator' must point to the aggregation value in the 'data' field", h3Query.Alias, query.Alias)
	}
	...
	params.isNumeratorBucket = sdl.IsBucketAggregateQuery(numeratorQuery.Field)
	if params.isNumeratorBucket {
		// get key path for numerator bucket aggregation
		v, ok := am["numerator_key"]
		if !ok {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' must contain 'numerator_key' argument for bucket aggregation", h3Query.Alias, query.Alias)
		}
		params.numeratorKey, ok = v.(string)
		if !ok || params.numeratorKey == "" {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'numerator_key' must be a string", h3Query.Alias, query.Alias)
		}
		...
		if params.numeratorKey == "" || strings.Contains(params.numeratorKey, ".") {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'numerator_key' must be a non-empty string", h3Query.Alias, query.Alias)
		}
	}
	params.isDenominatorBucket = sdl.IsBucketAggregateQuery(denominatorQuery.Field)
	if params.isDenominatorBucket {
		// get key path for denominator bucket aggregation
		v, ok := am["denominator_key"]
		if !ok {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' must contain 'denominator_key' argument for bucket aggregation", h3Query.Alias, query.Alias)
		}
		params.denominatorKey, ok = v.(string)
		if !ok || params.denominatorKey == "" {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'denominator_key' must be a string", h3Query.Alias, query.Alias)
		}
		...
		if params.denominatorKey == "" || strings.Contains(params.denominatorKey, ".") {
			return params, sdl.ErrorPosf(query.Position, "H3 query '%s' field '%s' argument 'denominator_key' must be a non-empty string", h3Query.Alias, query.Alias)
		}
	}
	return params, nil
}
...
func validateH3Query(query *ast.Field, vars map[string]any) (count int, err error) {
	...
	am := query.ArgumentMap(vars)
	if len(am) == 0 {
		return 0, sdl.ErrorPosf(query.Position, "H3 query must contain 'resolution' argument")
	}
	ri, ok := am["resolution"]
	if !ok {
		return 0, sdl.ErrorPosf(query.Position, "H3 query must contain 'resolution' argument")
	...
	res, ok := ri.(int64)
	if !ok || res < 0 || res > 15 {
		return 0, sdl.ErrorPosf(query.Position, "H3 query 'resolution' argument must be an integer between 0 and 15")
	}

	// check at least one data field
	ss := engines.SelectedFields(query.SelectionSet)
	if ss.ForName(base.H3DataFieldName) == nil {
		return 0, sdl.ErrorPosf(query.Position, "H3 query must contain at least one 'data' field")
	}
	count = len(ss)
...
func validateDistributionByField(query, field *ast.Field, sourceFields []string, vars map[string]any) error {
	...
	if field == nil ||
		...
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' type must be '%s'", field.Alias, query.Alias, base.DistributionTypeName)
	}
	am := field.ArgumentMap(vars)
	if len(am) == 0 {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain arguments", field.Alias, query.Alias)
	}
	v, ok := am["numerator"]
	if !ok {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain 'numerator' argument", field.Alias, query.Alias)
	}
	numerator, ok := v.(string)
	if !ok || numerator == "" {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' argument must be a string", field.Alias, query.Alias)
	}
	ss := engines.SelectedFields(query.SelectionSet)
	nq, np := pathToAggregationQuery(ss, numerator, sourceFields)
	if nq == nil || !sdl.IsAggregateQuery(nq.Field) {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' argument path '%s' must point to the aggregation value in the '%v' field", field.Alias, query.Alias, numerator, sourceFields)
	}
	numeratorPath := strings.TrimPrefix(numerator, np+".")
	err := checkDistributedByPath(numeratorPath, "", nq.Field)
	if err != nil {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' argument path '%s': %v", field.Alias, query.Alias, numeratorPath, err)
	}

	v, ok = am["denominator"]
	if !ok {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain 'denominator' argument", field.Alias, query.Alias)
	}
	denominator, ok := v.(string)
	if !ok || denominator == "" {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'denominator' argument must be a string", field.Alias, query.Alias)
	}
	dq, dp := pathToAggregationQuery(ss, denominator, sourceFields)
	if dq == nil || !sdl.IsAggregateQuery(dq.Field) {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'denominator' argument path '%s' must point to the aggregation value in the '%v' field", field.Alias, query.Alias, denominator, sourceFields)
	}
	denominatorPath := strings.TrimPrefix(denominator, dp+".")

	err = checkDistributedByPath(denominatorPath, "", dq.Field)
	if err != nil {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'denominator' argument path '%s': %v", field.Alias, query.Alias, denominatorPath, err)
	}

	if numeratorPath == denominatorPath {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' and 'denominator' argument paths must not be the same", field.Alias, query.Alias)
	}
	if !sdl.IsAggregateQuery(nq.Field) || !sdl.IsAggregateQuery(dq.Field) {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' or 'denominator' argument paths must point to aggregate queries", field.Alias, query.Alias)
	}
	return nil
}
...
func validateDistributionByBucketField(query, field *ast.Field, sourceFields []string, vars map[string]any) error {
	...
	if field == nil ||
		...
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' type must be '%s'", field.Alias, query.Alias, base.BucketDistributionTypeName)
	}
	am := field.ArgumentMap(vars)
	if len(am) == 0 {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain arguments", field.Alias, query.Alias)
	...
	if sdl.IsBucketAggregateQuery(nq.Field) {
		v = am["numerator_key"]
		if v == nil {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain 'numerator_key' argument", field.Alias, query.Alias)
		}
		numeratorKey, ok := v.(string)
		if !ok || numeratorKey == "" {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator_key' argument must be a string", field.Alias, query.Alias)
		}
		if !strings.HasPrefix(numeratorKey, np+".") {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator_key' argument path '%s' must point to the key in the numerator bucket aggregation", field.Alias, query.Alias, numeratorKey)
		}
	...
	denominatorPath := strings.TrimPrefix(denominator, dp+".")
	denominatorKeyPath := ""
	if sdl.IsBucketAggregateQuery(dq.Field) {
		v = am["denominator_key"]
		if v == nil {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field must contain 'denominator_key' argument", field.Alias, query.Alias)
		}
		denominatorKey, ok := v.(string)
		if !ok || denominatorKey == "" {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'denominator_key' argument must be a string", field.Alias, query.Alias)
		}
		if !strings.HasPrefix(denominatorKey, dp+".") {
			return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'denominator_key' argument path '%s' must point to the key in the denominator bucket aggregation",
	...
	if !sdl.IsBucketAggregateQuery(nq.Field) &&
		!sdl.IsBucketAggregateQuery(dq.Field) {
		return sdl.ErrorPosf(field.Position, "%s H3 query '%s' field 'numerator' or 'denominator' argument paths must point to bucket aggregations", field.Alias, query.Alias)
	}
...
func checkDistributedByPath(value, key string, query *ast.Field) error {
	if value == "" {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path must not be empty", query.Alias)
	}
	if sdl.IsAggregateQuery(query) {
		if key != "" {
			return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must not contain a key, only value", query.Alias, value)
		}
		vf := engines.SelectedFields(query.SelectionSet).ForPath(value)
		if vf == nil {
			return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' not found in the aggregation query", query.Alias, value)
		}
		if len(vf.Field.SelectionSet) != 0 {
			return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must point to an aggregated value, not a selection set", query.Alias, value)
		}
		if vf.Field.Definition.Type.NamedType == "" ||
			...
			return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must be a Float, BigInt or Int", query.Alias, value)
		}
		return nil
	}

	if !sdl.IsBucketAggregateQuery(query) {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must point to an aggregate query or bucket aggregation", query.Alias, value)
	}
	vf := engines.SelectedFields(query.SelectionSet).ForPath(value)
	if vf == nil {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' not found in the bucket aggregation", query.Alias, value)
	}
	if vf.Field.Definition.Type.NamedType == "" ||
		vf.Field.Definition.Type.NamedType != "Float" &&
			vf.Field.Definition.Type.NamedType != "Int" &&
	...
	if vf == nil || vf.Field.Definition.Name != base.AggregateFieldName {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must point to an aggregated value in the bucket aggregation", query.Alias, value)
	}
	if key == "" || strings.Contains(key, ".") {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' must contain a key in the bucket aggregation", query.Alias, value)
	}
	kf := engines.SelectedFields(query.SelectionSet).ForPath(key)
	if kf == nil || kf.Field.Name != base.AggregateKeyFieldName {
		return sdl.ErrorPosf(query.Position, "H3 query '%s' field 'numerator' or 'denominator' argument path '%s' key not found in the bucket aggregation", query.Alias, key)
	}
...
func pathToAggregationQuery(ss engines.SelectionSet, path string, sourceFields []string) (query *engines.SelectedField, queryPath string) {
	...
	if sdl.IsAggregateQuery(query.Field) || sdl.IsBucketAggregateQuery(query.Field) {
		return query, pp[0]
	}
